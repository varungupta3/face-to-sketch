...
def generator_v2(x, batch_size, is_train, reuse):
  ...
  with tf.variable_scope('GEN', reuse=reuse) as vs:
    ...
    with tf.variable_scope('deconv1', reuse=reuse):
      hidden_num /= 2
      x = t_conv_factory(x, hidden_num,[batch_size,4,4,512] ,3, 1, is_train, reuse)
      print (x.shape)

    with tf.variable_scope('deconv2', reuse=reuse):
      hidden_num /= 2
      x = t_conv_factory(x, hidden_num,[batch_size,8,8,256] ,3, 1, is_train, reuse)
      print (x.shape)

    with tf.variable_scope('deconv3', reuse=reuse):
      hidden_num /= 2
      x = t_conv_factory(x, hidden_num,[batch_size,16,16,128] ,3, 1, is_train, reuse)
      print (x.shape)

    with tf.variable_scope('deconv4', reuse=reuse):
      hidden_num /= 2
      x = t_conv_factory(x, hidden_num,[batch_size,32,32,64] ,3, 1, is_train, reuse)
      print (x.shape)
